...
def setup_files():
    ...
    test_program = """
  LOAD_CONST 0x01 0       # Устанавливаем в регистр 0x01 начальный адрес первого вектора (адрес = 0).
LOAD_CONST 0x02 5       # Устанавливаем в регистр 0x02 начальный адрес второго вектора (адрес = 5).
LOAD_CONST 0x03 10      # Устанавливаем в регистр 0x03 начальный адрес для записи результата (адрес = 10).
LOAD_CONST 0x04 5       # Устанавливаем в регистр 0x04 длину векторов (длина = 5).

LOAD_CONST 0x05 0       # Инициализируем счётчик (регистр 0x05) с начальным значением 0.

LOOP_START:             # Метка начала цикла.
READ_MEM 0x06 [0x01+0x05]   # Считываем значение из первого вектора по адресу (0x01 + значение счётчика 0x05).
READ_MEM 0x07 [0x02+0x05]   # Считываем значение из второго вектора по адресу (0x02 + значение счётчика 0x05).
BITWISE_SHIFT_RIGHT 0x08 [0x06+0x07]  # Выполняем побитовый сдвиг вправо результата суммы значений (0x06 + 0x07).
WRITE_MEM [0x03+0x05] 0x08  # Записываем результат в память по адресу (0x03 + значение счётчика 0x05).
    """
